...
int getNextInt(ref int i)
{
	...
	else if(isVariable(mem[i].cData) && getVarType(mem[i].cData) == 1)
	{
...
string getVarInt(string varName)
{
	return strVariable[varName];
}

int getVarStr(string varName)
{
